...
import { getPortfolio, getProfile } from '../services/api';
...
export default function Dashboard() {
  ...
  useEffect(() => {
    Promise.all([getPortfolio(), getProfile()])
      .then(([p, pr]) => { setPortfolio(p.data); setProfile(pr.data); })
  ...
  const totalInvested = profile?.stats?.total_invested || 0;
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 xl:grid-cols-4 gap-4">
        ...
        <StatCard icon={TrendingUp} label="Total Invested"        value={fmt(totalInvested)} color="emerald" />
        <StatCard
